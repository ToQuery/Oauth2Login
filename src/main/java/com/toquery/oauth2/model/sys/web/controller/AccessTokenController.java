...
public class AccessTokenController {
    ...
    public HttpEntity token(HttpServletRequest request) throws URISyntaxException, OAuthSystemException {
        ...
        try {
            ...
            //检查提交的客户端id是否正确
            if (!authCache.checkClientId(oauthRequest.getClientId())) {
            ...
            // 检查客户端安全KEY是否正确
            if (!authCache.checkClientSecret(oauthRequest.getClientSecret())) {
            ...
            if (oauthRequest.getParam(OAuth.OAUTH_GRANT_TYPE).equals(GrantType.AUTHORIZATION_CODE.toString())) {
                if (!authCache.checkAuthCode(authCode)) {
                    OAuthResponse response = OAuthASResponse.errorResponse(HttpServletResponse.SC_BAD_REQUEST)
            ...
            authCache.addAccessToken(accessToken, authCache.getUsernameByAuthCode(authCode));
            ...
            OAuthResponse response = OAuthASResponse
                    ...
                    .setExpiresIn(String.valueOf(authCache.getExpireIn()))
                    .buildJSONMessage();
