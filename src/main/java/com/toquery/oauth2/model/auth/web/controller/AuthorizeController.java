...
public class AuthorizeController {
    ...
    @Autowired
    private IAuthClientService oAuthService;
        ...
        try {
            ...
            if (!subject.isAuthenticated()) {
                if (!login(subject, request)) {//登录失败时跳转到登陆页面
                    model.addAttribute("client", oAuthService.getByClientId(oauthRequest.getClientId()));
                    return "oauth2login";
                }
            }
            ...
            if (responseType.equals(ResponseType.CODE.toString())) {
                ...
                authCache.addAuthCode(authorizationCode, username);
            }
